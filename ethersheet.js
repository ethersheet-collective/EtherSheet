 ...
 * EtherSheetService
 ***********************************************/
exports.EtherSheetService = function EtherSheetService(){
...
EtherSheetService.prototype.add_user_to_room = function(user){
  EtherSheetService._sheets[sheet_id] = EtherSheetService._sheets[sheet_id] || {count:0, users:{}};
  EtherSheetService._sheets[sheet_id].count++;
  EtherSheetService._sheets[sheet_id].users[user.token] = socket.udata;
...
EtherSheetService.prototype.remove_user_from_room = function(user){
  EtherSheetService._sheets[user.sheet_id].count--;
  delete(EtherSheetService._sheets[user.sheet_id].users[user.user_id]);
}

EtherSheetService.prototype.get_color = function(){
  idx = Math.floor(Math.random() * 100);  
  return EtherSheetService.colors[idx % colors.length]
};
...
EtherSheetService.prototype.create_sheet = function(sheet_id){
  ...
  EtherSheetService.sql.query(
    ...
    [sheet_id, EtherSheetService._default_sheetdata],
    function(err, results, fields){
      if(err) {
        throw err;
      }
      EtherSheetService.sheet_data = {sheetid: sheet_id, sheetdata: EtherSheetService._default_sheetdata} ;
      es.emit('sheet_ready');
    }
  );
}; 
