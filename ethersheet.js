...
es.add_user_to_room = function(socket, user, sheet_id){
  es._sheets[sheet_id] = es._sheets[sheet_id] || {count:0, users:{}};
  es._sheets[sheet_id].count++;
  socket.udata = {};
  console.log(socket.udata);
  socket.udata.user = user;
  socket.udata.sheet_id = sheet_id;
  es._sheets[sheet_id].users[user.token] = socket.udata;
...
/***********************************************
 * Express HTTP Server Initialization
 ***********************************************/
...
io.sockets.on('connection', function(socket){
  ...
  socket.on('disconnect', function(){
    ...
    es._sheets[socket.udata.sheet_id].count--
    delete(es._sheets[socket.udata.sheet_id].users[socket.udata.user_id])
