...
Sheet.prototype.rowAt = function(index,cb){
  ...
  sheet.db.getSheet(this.id,function(err,data){
    if(err) cb(err);
    if(index >= data.row_index.length) return cb(new Error("row index out of bounds"));
    cb(null,data.row_index[index]);
  });
};
...
Sheet.prototype.colAt = function(index,cb){
  ...
  sheet.db.getSheet(this.id,function(err,data){
    if(err) cb(err);
    if(index >= data.col_index.length) return cb(new Error("col index out of bounds"));
    cb(null,data.col_index[index]);
  });
};
