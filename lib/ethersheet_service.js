...
EtherSheetService.prototype.getSheetCollection = function(collection_id,cb){
  var url = "http://localhost:7007/doc/es_default_collection/"+collection_id.toString();
  request(url,function(err,res,body){
    if (err) return cb(err);
    if (res.statusCode == 404) return cb(null,'null');
    var doc_string = '{"type":"'+res.headers['x-ot-type']+'","v":'+res.headers['x-ot-version']+',"snapshot":'+body+'}';
    cb(null,doc_string);
  });
};
